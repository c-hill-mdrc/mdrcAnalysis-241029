...
CharTreat <- 
hetreg_extract(sim_data_robust_reg_PC
              ,.treatment  = "treat2"
              ,.dependents = c("employed_01", "employed_02")
              ,.subgroup   = "eduLevel"
              )
...
RA_CODE <- 
hetreg_extract(sim_data_robust_reg_RACODE
              ,.treatment  = "RA_CODE"
              ,.dependents = c("employed_01", "employed_02")
              ,.subgroup   = "eduLevel"
              )
...
CompareRR <- 
  ...
  dplyr::left_join(robustreg_withoutCluster %>% dplyr::select(UniqueId, StdErr_1_0), by = "UniqueId") %>% 
